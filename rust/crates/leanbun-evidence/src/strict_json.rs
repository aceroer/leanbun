...
pub fn parse_strict_json(text: &str) -> Result<StrictJson, EvidenceError> {
...
impl Parser<'_> {
    fn parse_value(&mut self, depth: usize) -> Result<StrictJson, EvidenceError> {
    ...
    fn parse_array(&mut self, depth: usize) -> Result<StrictJson, EvidenceError> {
        self.consume(b'[')?;
        self.skip_whitespace();
    ...
    fn parse_object(&mut self, depth: usize) -> Result<StrictJson, EvidenceError> {
        self.consume(b'{')?;
        self.skip_whitespace();
    ...
    fn parse_escape(&mut self, output: &mut Vec<u8>) -> Result<(), EvidenceError> {
        let escaped = self
            .next()
    ...
    fn hex_quad(&mut self) -> Result<u16, EvidenceError> {
        let mut value = 0_u16;
        for _ in 0..4 {
    ...
    fn parse_number(&mut self) -> Result<JsonNumber, EvidenceError> {
    ...
    fn literal(&mut self, expected: &[u8]) -> Result<(), EvidenceError> {
        if self
            .input
    ...
    fn consume(&mut self, expected: u8) -> Result<(), EvidenceError> {
        self.skip_whitespace();
    ...
    fn error(&self, detail: &str) -> EvidenceError {
        EvidenceError::new(
